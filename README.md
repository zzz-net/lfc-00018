...
系统首次启动时自动创建以下测试账号（账号配置单一来源：`api/config/seed.config.ts`）：
...
权限矩阵单一来源：`api/config/seed.config.ts` 中的 `ROLE_PERMISSIONS`
...
|------|--------|--------|--------|------|
| `design:import` | ✅ | ❌ | ❌ | 导入设计稿 |
| `design:view_all` | ✅ | ✅ | ✅ | 查看设计稿 |
| `design:claim` | ✅ | ✅ | ❌ | 认领设计稿 |
| `design:review` | ✅ | ✅ | ❌ | 评审（通过/退回） |
| `design:resubmit` | ✅ | ❌ | ✅ | 重新提交 |
| `design:comment` | ✅ | ✅ | ✅ | 添加评论 |
| `design:export` | ✅ | ❌ | ❌ | 导出评审纪要 |
| `user:manage` | ✅ | ❌ | ❌ | 管理用户（增删改） |
| `user:view` | ✅ | ✅ | ✅ | 查看用户列表 |
| `user:import` | ✅ | ❌ | ❌ | 批量导入用户 |
| `batch_task:view_list` | ✅ | ✅ | ✅ | 查看批量任务列表 |
| `batch_task:view_detail` | ✅ | ❌ | ❌ | 查看批量任务详情 |
| `batch_task:view_summary` | ✅ | ✅ | ✅ | 查看批量任务摘要 |
| `batch_task:create` | ✅ | ❌ | ❌ | 创建批量任务 |
| `batch_task:execute` | ✅ | ❌ | ❌ | 执行/更新批量任务 |
| `batch_task:delete` | ✅ | ❌ | ❌ | 删除批量任务 |
| `batch_task:export_conflicts` | ✅ | ❌ | ❌ | 导出冲突 CSV |
| `admin_log:view` | ✅ | ❌ | ❌ | 查看管理员操作日志 |

### 角色说明

- **管理员**: 系统管理、设计稿导入、用户管理、批量任务、日志查看
- **评审人**: 认领设计稿、评审（通过/退回）、添加评论
- **提交者**: 查看自己的设计稿、重新提交、添加评论

## 启动模式

系统支持四种启动模式（通过环境变量 `STARTUP_MODE` 指定）：

| 模式 | 说明 | 行为 |
|------|------|------|
| `first_launch` | 首次启动 | 自动创建演示账号和示例设计稿 |
| `restart` | 已有库重启 | 仅验证数据库版本，不创建数据 |
| `upgrade` | 旧库升级 | 执行迁移脚本，不破坏已有数据 |
| `test` | 测试环境 | 重置数据库为干净状态（仅测试用） |

默认自动检测：数据库不存在时为 `first_launch`，存在时为 `restart`。
...
所有验证脚本都位于 `scripts/` 目录下，作为 CI/CD 的一部分运行。

### 账号与权限验证

验证所有 seed 账号能正确登录，角色信息正确，权限控制正确：
...
- 登录后角色、姓名信息正确
- 非管理员访问受限接口被正确拦截（403）
- 未登录用户访问受限接口被正确拦截（401）

### README 账号一致性验证

验证 README 中的账号表格与 `seed.config.ts` 配置完全一致：

```bash
node scripts/verify-readme-accounts.mjs
```

一旦 README 账号信息与代码配置不一致，脚本直接失败，防止文档漂移。

### 数据库初始化与迁移验证

验证四种启动路径、数据持久化和迁移正确性：
...
测试内容：
- 首次启动（first_launch）：正确创建用户和示例数据
- 已有库重启（restart）：不重复创建数据
- 数据持久化：重启后数据完整保留
- 旧库升级（upgrade）：迁移脚本正确执行，不破坏已有数据

### 端到端验证脚本

验证登录、导入、冲突检测、错误提示等完整链路：
...
一次性运行：README 一致性检查 → 数据库验证 → 账号权限验证 → 端到端测试。任何一个环节失败立即终止。
